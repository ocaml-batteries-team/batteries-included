...
sig
  ...
  val gen : ('a, 'b, 'c) t -> 'a BatGen.t
  (** [gen e] returns an generation on the elements of [e].
    The order of generation is unspecified.*)
...
module Array1 : sig
  ...
  val gen : ('a, 'b, 'c) t -> 'a BatGen.t
  (** [Array1.gen e] returns an generation on the elements of [e].
      Contrarily to the multi-dimensional case, order of elements is
      specified: elements are in sequential order, from smaller to
      larger indices. *)

  val of_gen : ('a, 'b) kind -> 'c layout -> 'a BatGen.t -> ('a, 'b, 'c) t
  (** [Array1.of_gen kind layout gen] returns a new one-dimensional
      big array of kind [kind] and layout [layout], with elements taken
      from the generation [gen] in order.
