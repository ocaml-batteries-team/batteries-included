...
let ofs_of_layout (layout : _ Bigarray.layout) =
  match (Obj.magic layout : int) with
  | 0 -> 0 (* keep these magic constants in sync with bigarray.ml *)
  | 0x100 -> 1 (* and transitively with caml_ba_layout in bigarray.h  *)
  | _ -> failwith "Unknown layout"
# 103 "src/batBigarray.mlv"

# 107 "src/batBigarray.mlv"
