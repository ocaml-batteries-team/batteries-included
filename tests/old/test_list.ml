...
let rand_list n =
  let elem _ = Random.int 5000 in
  List.init n elem
 
let count_f (f_cat, desc, max_n, f) seed =
  let test_n n =
    let list = rand_list n in (* one list for both *)
...
let tests =
  [ "nth_g", ("Gallium", "Nth", max_int,
   ...
   (fun n l -> ignore(List.map (( * )2) l)));
    "mapx_g", ("Gallium", "Map: /2 or *3+1", 100_000,
    (fun n l -> ignore(Legacy.List.map (count_fall 0) l)));
    "mapx_e", ("Extlib", "Map: /2 or *3+1", max_int,
    (fun n l -> ignore(List.map (count_fall 0) l)));
    "folr_g", ("Gallium", "Fold_right: sum", 100_000,
    (fun n l -> ignore(Legacy.List.fold_right (+) l 0)));
    "folr_e", ("Extlib", "Fold_right: sum", max_int,
   (fun n l -> ignore(List.fold_right (+) l 0)));
    "folrx_g", ("Gallium", "Fold_right: sum fall", 100_000,
    (fun n l -> ignore(Legacy.List.fold_right acc_cf l 0)));
    "folrx_e", ("Extlib", "Fold_right: sum fall", max_int,
    (fun n l -> ignore(List.fold_right acc_cf l 0)));
    "map2_g", ("Gallium", "Map2: sum", 100_000,
   (fun n l -> ignore(Legacy.List.map2 (+) l l)));
    "map2_e", ("Extlib", "Map2: sum", max_int,
   (fun n l -> ignore(List.map2 (+) l l)));
...
let () =
  ...
  let get_f n = List.assoc n tests in
  let do_test n =
    try count_f (get_f n) !seed
    with Done_measuring -> () in
  List.iter do_test (List.rev !todo)
